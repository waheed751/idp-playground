...
#st.caption("Select document type → upload PDF → extract structured data.")
st.divider()
...
with col_left:
    ...
    if selected != st.session_state["selected_doc_type"]:
        ...
        st.session_state["structure_input"] = json.dumps(
            DOC_TYPES[selected]["structure"], indent=2
        )
    ...
    # ── Structure editor ───────────────────────────────────────────────────────
    st.markdown("### Step 2 — Edit structure (optional)")
    st.caption("Default structure for this document type. Edit freely.")
    ...
    run_validation = st.checkbox(
        ...
        value=False,
        help="Runs to check if PDF matches selected document type."
    )
    ...
    if uploaded_file:
        ...
        if run_validation and st.session_state["validation_result"] is None:
            if st.button("Validate document", use_container_width=True):
                with st.spinner("Running validation..."):
                    ...
                    val_result = validate_doc_type(
                        pdf_bytes, selected, doc_config["classifier_label"]
                    )
        ...
        if val:
            if val["error"]:
                ...
                st.warning(
                    f"⚠️ Detected as **{val['detected']}** but you selected **{selected}**. Proceed anyway?"
                )
        ...
        if can_upload:
            if st.button("Upload file to process", use_container_width=True, type="primary"):
                with st.spinner(f"Uploading ..."):
                    result = upload_fileobj_to_s3(
                        uploaded_file, uploaded_file.name, doc_config["s3_folder"]
                    )
                if result["success"]:
    ...
    if st.session_state["uploaded_s3_uri"]:
        st.divider()
        st.markdown("### Step 4 — Start Process")
        st.info(f"**File:** `{st.session_state['uploaded_filename']}`", icon="✅")
        st.caption(f"Type: `{doc_config['api_type']}` ")

        if st.button("Process", type="primary", use_container_width=True, disabled=not structure_valid):
            s3_uri = st.session_state["uploaded_s3_uri"]

            with st.spinner("Submitting job..."):
                try:
                    job_response = submit_ocr_job(s3_uri, doc_config["api_type"], parsed_structure)
